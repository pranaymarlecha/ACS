// For this component to work correctly, make sure your project is set up with Tailwind CSS.
// You can find installation instructions here: https://tailwindcss.com/docs/installation
...
const InputField = ({ label, type = 'number', value, onChange, name, placeholder = '', min = 0, step = 1 }) => (
  <div className="mb-4">
...
export default function App() {
  ...
  const callGeminiAPI = async (prompt) => {
    ...
    // The API key is managed by the execution environment. Do not change this line.
    const apiKey = ""; 
    ...
    try {
      ...
      const errorMessage = `Error: Could not generate content. ${error.message}`;
    ...
    <div className="min-h-screen bg-gray-50 font-sans text-gray-800 p-4">
      <header className="bg-white shadow-sm rounded-lg mb-6">
        <div className="max-w-7xl mx-auto py-4 px-4 sm:px-6 lg:px-8">
          <h1 className="text-2xl md:text-3xl font-bold text-gray-900">ACS Diagnostic & Risk Assessment Tool</h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="space-y-6">
            ...
            <div className="flex space-x-4">
                <button onClick={calculateScoresAndDiagnosis} className="w-full bg-indigo-600 text-white py-3 px-4 rounded-md hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 font-semibold transition-colors">
                    Analyze Patient Data
                </button>
                <button onClick={resetForm} className="w-full bg-gray-200 text-gray-700 py-3 px-4 rounded-md hover:bg-gray-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-400 font-semibold transition-colors">
                    Reset
                </button>
            </div>
          </div>
                ...
                {results ? (
                  <div className="space-y-6">
                      ...
                      <div className="border-t pt-4 space-y-4">
                          ...
                          <button onClick={handleGenerateSummary} disabled={isGenerating} className="w-full bg-blue-500 text-white py-2 px-4 rounded-md hover:bg-blue-600 disabled:bg-blue-300 font-semibold flex items-center justify-center transition-colors">
                              {isGenerating && !generatedContent.summary ? 'Generating...' : 'Generate Clinical Summary'}
                          </button>
                          
                          {generatedContent.summary && (
                              <div className="bg-gray-100 p-3 rounded-md">
                                  <div className="w-full text-sm bg-gray-100 whitespace-pre-wrap p-2 rounded">{generatedContent.summary}</div>
                          ...
                          {results.diagnosis.includes('Low Probability') && (
                              <>
                                  <button onClick={handleGenerateDifferentials} disabled={isGenerating} className="w-full bg-teal-500 text-white py-2 px-4 rounded-md hover:bg-teal-600 disabled:bg-teal-300 font-semibold flex items-center justify-center transition-colors">
                                      {isGenerating && !generatedContent.differentials ? 'Generating...' : 'Suggest Differential Diagnoses'}
                                  </button>
                                  {generatedContent.differentials && (
                                       <div className="bg-gray-100 p-3 rounded-md">
                                           <div className="w-full text-sm bg-gray-100 whitespace-pre-wrap p-2 rounded">{generatedContent.differentials}</div>
                                       </div>
                                  )}
                              </>
                          )}
